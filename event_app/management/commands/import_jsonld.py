...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--commit', action='store_true', help='Actually write to DB (dry-run by default)')
    ...
    def handle(self, *args, **options):
        ...
        counters = {'venue': 0, 'category': 0, 'event': 0, 'offer': 0, 'skipped': 0, 'errors': 0}
        ...
        for idx, item in enumerate(data, start=1):
            ...
            if options.get('commit'):
                    ...
                    with transaction.atomic():
                        venue_obj, v_created = Venue.objects.get_or_create(
                            ...
                            defaults={
                                ...
                                'latitude': Decimal(str(vn.get('latitude'))) if vn.get('latitude') not in (None, '') else Decimal('0.0'),
                                'longitude': Decimal(str(vn.get('longitude'))) if vn.get('longitude') not in (None, '') else Decimal('0.0'),
                            }
                        )
                        if v_created:
                            counters['venue'] += 1
                ...
                self.stdout.write(f'[{idx}/{total}] ext_id={ev.get("ext_id")} event_name={ev.get("event_name")!r} offer={bool(offer)}')
