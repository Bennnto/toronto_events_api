...
from .forms import Login_Form, Registry_Form
from .models import Event
from .serializers import EventSerializer
from .throttles import AdminThrottle, AuthThrottle, BaseThrottle
...
@extend_schema_view(
    ...
    create=extend_schema(tags=["Admin Events"], description="Create a new event."),
    update=extend_schema(tags=["Admin Events"], description="Update exisiting events."),
    partial_update=extend_schema(tags=["Admin Events"], description="Partials update existing events."),
    destroy=extend_schema(tags=["Admin Events"], description="Delete events from database"),
)
...
def user_register(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            user = form.save()
            email_message = (
                "Your Toronto Event APIs Account Created Successfully\n\n"
                "To Obtain Access Token and Refresh Token Please Send\n"
                f"Your Credential username={user.username} and your password to Token Endpoint.\n\n"
